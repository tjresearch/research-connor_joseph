...
def predictImg(img):
    img = open_image(img)
    path = Path("C:/Users/Conno/OneDrive/Documents/Sys Lab/webapp")
    defaults.device = torch.device('cpu')
    learn = load_learner(path) # accesses export.pkl
...
if img != None:
    ...
    if st.checkbox("Yes! Do better!"):
        dtype = st.radio("Which Disaster is in the Picture?", ["earthquake", "hurricane", "fire", "flooding"])
